...
public class GameSerializer {
    ...
    public GameState load(File loadFile) {
        try (Scanner scanner = new Scanner(loadFile)) {
            try {
                ...
                ChessPlayer turn = ChessPlayer.valueOf(scanner.next());
                ...
                while (scanner.hasNextLine()) {
                    ...
                    ChessTile tile = new ChessTile(rank, file);
                    ChessPlayer pl = ChessPlayer.valueOf(p);
        ...
        try (PrintWriter writer = new PrintWriter(saveFile)) {
            ...
            for (int rank = 0; rank < 8; rank++) {
                for (int file = 0; file < 8; file++) {
                    Piece piece = state.board.getPieceAt(new ChessTile(rank, file));
     ...
     */
    private Piece loadPiece(ChessBoard board, ChessPlayer player, String name) {
        switch (name) {
            case "Pawn":
