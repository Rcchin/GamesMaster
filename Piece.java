...
abstract public class Piece {
    ...
    public Piece(ChessBoard board, ChessPlayer player) {
        this.board = board;
        this.player = player;
    }
     ...
     */
    public List<ChessTile> getAllMoves(ChessTile from) {
        List<ChessTile> moves = new ArrayList<>();

        for (int rank = 0; rank < 6; rank++) {
            for (int file = 0; file < 6; file++) {
                ChessTile t = new ChessTile(rank, file);
    ...
    public List<ChessTile> getAllSafeMoves(ChessTile from) {
    	List<ChessTile> moves = new ArrayList<>();
     ...
     */
    protected boolean canMove(ChessTile t) {
     ...
     */
    protected boolean canCapture(ChessTile t) {
     ...
     */
    protected boolean canMoveOrCapture(ChessTile t) {
     ...
     */
    protected boolean isPathBlocked(ChessTile from, ChessTile to) {
        ...
        while (rank != to.getRank() || file != to.getFile()) {
            if (getBoard().isOccupied(new ChessTile(rank, file))) {
                return true;
            }
     ...
     */
    abstract public boolean canMove(ChessTile from, ChessTile to);
}
